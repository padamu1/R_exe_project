...
{
	// TODO: 파일 열기 컨트롤
	static TCHAR BASED_CODE szFilter[] = _T("엑셀파일(*.csv) | *.csv |모든파일(*.*)|*.*||");
...
void CRprojectDlg::OnBnClickedButtDurup()
{
	...
	system("");
	system("%var% R CMD BATCH test.R %FILENAME%");
	system("pause");
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
}
...
void CRprojectDlg::OnBnClickedButtBanana()
{
	...
	if (FILENAME == "") {
		...
		batch << "@echo OFF\n";
		batch << "for /f \"tokens=* usebackq\" %%a in (`^WHERE /R C:\\\\"<<"\"Program Files\" R.exe`) do set var=%%a\n";
		batch << "echo " << FILENAME << "\n";
		batch << "pause\n";
		batch.close();
		system("banana.bat");
